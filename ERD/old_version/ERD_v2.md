...
- `third_party_needed`이면서 승인된 보호자·지원인력가 없을 때만 연결 권유를 표시한다. `care_level_notice_dismissals.dismissed_until`까지 숨기며 서비스 자체는 차단하지 않는다.
...
- `third_party_needed`이고 승인된 보호자·지원인력이 한 명 이상이면 최소 한 명에게 돌봄 알림을 유지한다. 승인 관계가 0명이면 REQ-007a를 우선 적용한다. 마지막 연결 해제는 승인 절차를 거치며 승인 후 0명이 되면 REQ-007a를 즉시 활성화한다.
- `education_profiles.assigned_educator_id`는 역할이 `caregiver`, `life_support_worker`, `social_worker`인 사용자만 허용하며 `guardian`은 금지한다. 이 규칙은 애플리케이션 검증과 DB 제약으로 강제한다.
