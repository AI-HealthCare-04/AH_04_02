...
> 버전: v7 (2026-07-03). v6 대비 변경 내역은 `docs/revision_logs/ERD` 참고.
...
erDiagram
    ...
    users ||--o{ care_relation_invitations : "medication_subject_id"
    users ||--o{ care_relation_invitations : "matched_user_id"
    users ||--o{ care_relation_invitations : "created_by"
    users ||--o{ care_relations : "medication_subject_id"
    users ||--o{ care_relations : "related_user_id"
    users ||--o{ care_relations : "approved_by"
    users ||--o{ care_relations : "revocation_requested_by"
    users ||--o{ care_relations : "revocation_decided_by"
    care_relation_invitations ||--o| care_relations : "accepted_invitation"
    ...
    users {
        ...
        string special_notes "nullable; 특이사항"
    ...
    care_relations {
        ...
        bigint medication_subject_id FK
        bigint related_user_id FK "NOT NULL"
        enum relation_type "guardian|caregiver|life_support_worker|social_worker"
...
- 초대는 `care_relation_invitations`, 승인된 관계는 `care_relations`에 분리한다. 원문 초대 토큰 대신 해시를 저장하고 미가입 수신자는 인증 전화번호가 일치한 가입·로그인 후 수락한다.
- (v6) `trust_invitations`/`trust_relations`/`trustee_id`는 `care_relation_invitations`/`care_relations`/`related_user_id`로 이름을 바꿨다. `relation_type`이 `guardian` 외 `caregiver`·`life_support_worker`·`social_worker`도 포함하므로 관계 자체를 "보호자(trust)"로 부르지 않고 도메인 중립적인 "돌봄관계(care relation)"로 통일한다.
- (v7) `users.birthday`/`address`/`special_notes`는 회원가입 시 수집하는 인적사항이다. 주민등록번호는 개인정보보호법상 고유식별정보로 수집을 엄격히 제한하므로 대체 항목으로 `birthday`(생년월일)만 둔다. 세 필드 모두 nullable이며 값 검증(형식 등)은 애플리케이션 계층에서 수행한다.
- (v7) 회원가입 시 보호자 연락처를 입력하면(`role=medication_subject`인 경우) 서버가 같은 트랜잭션 흐름 안에서 `care_relation_invitations`에 초대 레코드를 자동 생성한다(`invited_phone`=입력한 보호자 연락처, `created_by`=신규 가입자 본인). 이는 REQ-002의 수동 초대 API(`POST /care-relations/invitations`)와 동일한 테이블·수락 절차를 공유하며 별도 스키마를 추가하지 않는다. "가입 시 연결할 기존 계정의 ID를 입력"하는 방식은 채택하지 않는다 — 로그인 식별자를 이메일·전화번호로 유지하기로 했으므로 사용자에게 노출되는 "계정 ID" 개념 자체가 없다.
- (v7, 범위 제외) 개인/단체 회원가입 구분은 이번 버전에서 다루지 않는다. `users.role`은 5개 역할 값만 가지며 `account_type`(individual/organization) 같은 구분 필드나 `organizations` 테이블은 없다 — 설계 미확정으로 보류된 백로그이며(`docs/draft_signup_mentoring_feedback_v1.md` 참고), 향후 추가 시 이 ERD도 별도 버전업이 필요하다.
- 해제 승인 `revocation_decision=approved`는 관계를 `revoked`로 변경하고, `revocation_decision=rejected`는 결정 이력을 남긴 뒤 `approved`로 복귀시킨다. (v6) API 요청 바디의 기존 `approve` boolean 필드명을 `care_relations.revocation_decision` 컬럼과 동일한 어휘인 `revocation_decision` enum(`approved|rejected`)으로 통일해 API 요청·응답·DB가 같은 용어를 쓰도록 맞췄다.
...
| 요구사항 | 테이블/필드 | API |
|---|---|---|
| REQ-001 | `users`, `auth_refresh_tokens` | `POST /auth/register`, `POST /auth/refresh` |
| REQ-002~004 | `care_relation_invitations`, `care_relations` | `/care-relations/invitations*`, `/care-relations*` |
| REQ-005~006 | `medication_subject_statuses`, `care_level_assessments.evaluated_at` | `POST /medication-subjects/{subject_id}/status`, `GET /medication-subjects/{subject_id}/care-level`, `GET /users/me/status`, `GET /users/me/care-level` |
...
| REQ-012~020 | `guide_results.cache_key`, `cache_expires_at` | `GET /medical-records/{record_id}/guide`, `POST /medical-records/{record_id}/guides` |
...
| REQ-007a | `care_level_notice_dismissals.dismissed_until` | `/medication-subjects/{subject_id}/care-level-notice*`, `/users/me/care-level-notice*` |
...
| REQ-035 | `users.status`, `withdrawal_requested_at`, `deactivated_at`, `scheduled_purge_at`, `privacy_purge_audits` | `DELETE /users/me`, `PATCH /users/me` |
| REQ-039 | `users.failed_login_count`, `locked_at`, `auth_temporary_codes` | `/auth/temporary-codes`, `/auth/temporary-login`, `/auth/password-reset/confirm` |
| REQ-040~044 | `education_profiles`, `education_support_logs`, `users.role` | `/medication-subjects/{subject_id}/education-*`, `/users/me/education-*`, `/educators/me/medication-subjects` |
| REQ-045 | `users`(`birthday`,`address`,`special_notes`) | `POST /auth/register`, `GET /users/me` |
| REQ-046 | `care_relation_invitations`(회원가입 경로에서 자동 생성) | `POST /auth/register`(`guardian_phone`, `guardian_relation_type`) |
| REQ-047 | 없음(응답 전용 값, 저장하지 않음) | `PATCH /medical-records/{record_id}/medications/{medication_id}`(`typo_suggestion`) |
| REQ-048 | `guide_results.guide_result_id`(존재 여부만 클라이언트가 참조) | `GET /medical-records/{record_id}/guide`, `/chat*` |
| REQ-049 | 없음(클라이언트 동작 규약, DB 영향 없음) | `POST /auth/refresh` |
