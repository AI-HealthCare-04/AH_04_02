...
> 버전: v9 (2026-07-21). v8 대비 변경 내역은 `docs/revision_logs/ERD/ERD_v8_to_v9_revision_log.md` 참고.
>
...
|---|---|---|---|---|
| REQ-001 | `caregivers`/`patients` | `POST /auth/login`, `GET /auth/token/refresh` | 완료 | 회원가입은 `/monitoring/patients`·`/monitoring/caregivers`가 겸함(`POST /auth/signup`은 2026-07-14 중복 코드로 제거됨) |
...
| REQ-021~022 | `chat_messages` | `/chat/*` | 완료(SSE 스트리밍은 미구현) | v8과 동일 |
...
| REQ-040~044 | 없음 | - | 미구현(교육·추적관리 테이블 자체 없음, v8과 동일) | |
