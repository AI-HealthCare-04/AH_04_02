...
export default function Login() {
  ...
  const proceedWithPatient = (
    ...
    // [2026-07-25 수정] 케어하는 환자가 여럿이어도 "어떤 환자를 볼지" 고르게 하지 않는다
    // — 모니터링 화면은 patient_id가 없으면 전체 요약을 보여주니, 첫 번째 환자를 다른
    // 환자 단위 화면들(스케줄 등)의 기본값으로만 조용히 저장해둔다.
    ...
    // 보호자/기관 계정의 홈은 환자 목록 관리가 아니라 전체 모니터링 요약 화면.
    navigate("/monitoring");
  };
  ...
  const handleSwitchAccount = async (account: RecentAccount) => {
    try {
      ...
      // 환자 본인 계정만 "오늘의 복약"(/dashboard)이 홈이고, 보호자/기관은 모니터링 요약이 홈.
      navigate(account.role === "patient" ? "/dashboard" : "/monitoring");
    } catch {
