...
DRUG_NAME_RE = re.compile(
    ...
    # 약품명은 그대로 매칭됨).
    # [2026-08-06 버그수정] "심비코트터부헬러"(Symbicort Turbuhaler)처럼 흡입기 디바이스명이
    # 브랜드명 뒤에 붙는 흡입제는 정/캡슐/액 같은 통상 제형어로 끝나지 않아 이 목록에
    # 하나도 안 걸렸다 — 표 전용 파서(parse_official_table_by_bbox)의 row_anchors가
    # DRUG_NAME_RE 매치를 행 판정 기준으로 쓰는데, 이 약이 매치되지 않으니 그 행 자체가
    # 통째로 인식에서 빠지고 다음 행(알레그라정)이 첫 행으로 잘못 승격되면서 원래 1행의
    # 투약량·횟수·총일수 값을 대신 흡수했다(실사용 재현: sample_03 천식·알레르기 비염
    # 처방전에서 심비코트터부헬러가 사라지고 그 값이 알레그라정 행에 섞임).
    # [2026-08-06 버그수정] "케토톱플라스타"처럼 패취제를 "패취/패치" 대신 "플라스타"
    # (plaster의 한글 표기 관례)로 표기하는 약품명도 위와 같은 이유로 통째로 누락됐다
    # (실사용 재현: sample_02 고지혈증·통증 처방전에서 케토톱플라스타가 사라짐).
    r"(?:정|캡슐|주|산|시럽|액|크림|연고|로션|겔|패취|패치|플라스타|점안|점이|(?<!질)환|스프레이|헬러"
...
# [2026-08-06 추가] "1흡입"(심비코트터부헬러 등 흡입기 계열의 1회 복용량 표기)이 빠져 있어
# 흡입형 약품의 dosage 셀 값이 인식되지 않았다.
DOSE_QTY_UNITS = "정|캡슐|캅셀|포|병|환|스틱|앰플|바이알|시린지|흡입|개(?!월)|매(?!월)"
...
DRUG_FORM_RE = re.compile(
    rf"({DOSE_QTY_UNITS}|주|산|시럽|액|크림|연고|로션|겔|패취|패치|플라스타|점안|점이|스프레이|헬러"
...
_DOSAGE_VALUE_RE = re.compile(
    r"^\d+(?:\.\d+)?\s*(?:정|캡슐|캅셀|포|병|환|스틱|앰플|바이알|시린지|흡입|개|매|mL|ml|g|단위|방울|분무|분사)$",
    re.IGNORECASE,
)
