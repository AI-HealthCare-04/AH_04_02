...
# [2026-07-27 버그수정] 패(패치/패취)·점(점안/점이)·로(로션)·스(스프레이)가 빠져 있어서
# 이 제형들은 HIRA 이름 매칭에서 "직접 일치"(_is_direct)로 인정받지 못하고 항상 더 약한
# 부분일치(contains)+최단이름 휴리스틱으로만 처리됐다.
# [2026-07-27 추가] 세(세립)·엘(엘릭서)·드(드링크)·앰(앰플)·바(바이알)·페(페이스트)·
# 필(필름)·트(트로키)·껌 도 같은 이유로 추가 — 과/좌/시/로는 기존 문자와 겹쳐 재사용됨.
_FORM_STARTERS = set("정캡주산시액이수분과좌크겔연패점로스세엘드앰바페필트껌")
...
_STRIP_RE = re.compile(
    ...
    r"|(?:정|캡슐|주사|주|산|시럽|액|환|겔|크림|연고|로션|패취|패치|점안|점이|스프레이"
