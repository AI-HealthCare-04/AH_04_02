...
**라우터 prefix**: `auth`(`/auth`), `records`(`/records`), `ocr`(`/ocr`), `rag`(`/rag`), `monitoring`(`/monitoring`), `chat`(`/chat`) 6개는 라우터명과 일치하는 prefix를 쓴다. **`care_router`만 prefix가 없어** 2·3·7절의 `/assessments`, `/invitations`, `/notification-settings`, `/patients/{id}/invitations`, `/trust/relations` 등이 다른 라우터와 달리 루트에 바로 노출된다 — RESTful 관점의 일관성 문제로 v8부터 지적됐으나 이번 버전에서도 아직 조치하지 않았다(다음 버전 후보).

> 버전: v9 (2026-07-21). v8 대비 변경 내역은 `docs/revision_logs/API명세서/API명세서_v8_to_v9_revision_log.md` 참고.
>
> **v9은 PR #61(챗봇 preset/dynamic LLM 유출·등록약 컨텍스트 누락 수정)과 PR #62(보호자→환자 초대 REQ-058 + MedicationRecord 통합 REQ-037 + 챗봇 출처표시/OCR 약품명/의약품명 매핑/복약가이드/검색우선순위/DUR속도 다수 수정)가 dev에 머지된 뒤 작성했다.** 이 두 PR로 1절(auth — 로그인 잠금·비밀번호 재설정·회원탈퇴가 실제로는 이미 구현돼 있었음이 이번에 처음 문서화됨), 2절(invitations — 보호자→환자 방향 추가), 5절(rag — source_refs 구조·검색 우선순위), 6절(chat — source_refs 응답 추가, LLM 게이팅 버그 수정), 7절(monitoring — MedicationRecord 승격, 알림 스케줄러 실체 확인)이 갱신됐다. 정합성 검토(`요구사항정의서_API명세서_정합성검토.docx`, 권순현)에서 지적된 REQ-009/011/032 서술 오류도 이번 버전에서 정정했다.
...
즉 **`guardian`과 `caregiver`는 서로 다른 테이블이 아니라 같은 `Caregiver` 테이블을 `relation_type` 값으로 구분**한다 — 권한 로직(`get_current_actor`/`require_actor_patient_access`)도 `relation_type`을 구분하지 않고 "이 환자에 연결된 Caregiver인가"만 검사한다. 지원인력과 guardian 사이에 코드 레벨의 권한 차등은 현재 없다(교육관리 기능이 없으므로 REQ-043의 "guardian은 교육자 불가" 제약도 아직 적용될 대상 자체가 없음, 8절 참고).
...
인증이 필요 없는 것은 의도된 예외로 ① 회원가입(`POST /monitoring/patients`, `POST /monitoring/caregivers`), ② 초대 링크 열람/수락/거절(`GET /invitations/{token}`, `POST /invitations/{token}/accept`·`/reject`), ③ 로그인/토큰 재발급/비밀번호 재설정(`POST /auth/login`, `GET /auth/token/refresh`, `POST /auth/password-reset/*`), ④ 세션 무관 단건 조회·헬스체크(`GET /ocr/ping`, `GET /ocr/drug-info`, `GET /rag/ping`)다. **`POST /auth/signup`은 2026-07-14에 `POST /monitoring/caregivers`와 중복되는 죽은 코드로 판단되어 제거됐다** — 회원가입은 `/monitoring/patients`·`/monitoring/caregivers` 둘로만 이뤄진다.
...
비동기 작업 접수(`202`)나 SSE 스트리밍은 없다 — 모든 요청은 동기적으로 처리되어 완료된 결과를 그대로 반환한다(REQ-021/028이 요구하는 SSE는 v8부터 이어지는 미구현 항목).
...
| GET | `/auth/token/refresh` | access_token 재발급(refresh 회전 + 재사용 탐지) | 쿠키 `refresh_token` | `200 LoginResponse`(동일 모양) | 쿠키 검증만 | 완료 |
...
| DELETE | `/records/{record_id}` | 처방전 소프트 삭제(`deleted_at`) — 연결된 `medication_schedules`도 함께 비활성화 | 경로 `record_id` | `200` | O | 완료 |
...
`education_profiles`, `education_support_logs`에 해당하는 테이블·엔드포인트가 `backend/models.py`/`backend/routers/`에 전혀 없다(v8과 동일). **REQ-043("guardian은 교육자 지정 불가")도 이 기능 자체가 없어 적용 대상이 없다** — 정합성 검토에서 "API명세서에 이 제약이 전혀 언급 없다"고 지적됐으나, 애초에 이 REQ가 다루는 기능이 미구현이라 코드 레벨 언급 자체가 성립하지 않는다(이 절 미구현 항목으로 이미 명시돼 있음).

## 범위 밖(v9 미문서화)

- **`patient_medications_router.py`**(2026-07-14 `main.py`에 등록, prefix 미확인)는 이번 v9의 조사 대상 7개 파일(auth/records/ocr/rag/monitoring/care/chat)에 포함되지 않아 이 문서에 없다 — 다음 버전에서 절을 신설해 다룰 것(Round-1 감사에서 발견된 스코프 갭).
