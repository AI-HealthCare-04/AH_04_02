...
> 버전: v4 (2026-07-02). v3 대비 변경 내역은 `docs/revision_logs/API명세서` 참고.
...
| POST | `/users/me/withdrawal/cancel` | 30일 내 탈퇴 취소 | 없음 | `200 {status:"active", purge_status:"cancelled"}` |
...
`DELETE /users/me` 처리 시 서버는 감사 기록(`privacy_purge_audits`)을 조회해 없으면 `purge_status:"scheduled"`로 새로 생성하고, 과거 취소로 남아있는 레코드가 있으면 재사용해 요청·삭제예정 시각을 갱신하고 `purge_status:"scheduled"`로 되돌린다(동일 사용자 재탈퇴 시 UK 충돌 방지). `POST /users/me/withdrawal/cancel`로 30일 내 탈퇴를 취소하면 계정을 `active`로 되돌리되, 이 감사 기록은 삭제하지 않고 `purge_status:"cancelled"`로 갱신해 취소 이력을 보존한다(v4 변경).
