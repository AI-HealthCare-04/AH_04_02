...
> 버전: v6 (2026-07-03). v5 대비 변경 내역은 `docs/revision_logs/API명세서` 참고.
...
## 1. 인증·사용자 (REQ-001, REQ-031, REQ-035, REQ-039, REQ-045, REQ-046, REQ-049)

| Method | Endpoint | 설명 | 요청 | 응답 |
|---|---|---|---|---|
| POST | `/auth/register` | 회원가입 | `{email?, phone?, password, name, role, birthday?, address?, special_notes?, guardian_phone?, guardian_relation_type?}` | `201 {user_id, role, invitation_id?}` |
...
| GET | `/users/me` | 내 계정 정보 조회 | 없음 | `200 {user_id, email, phone, name, role, status, birthday, address, special_notes, last_login_at}` |
...
(v6) 회원가입 인적사항으로 `birthday`(생년월일), `address`(주소), `special_notes`(특이사항)를 추가로 받는다. 모두 선택 입력(nullable)이다. 주민등록번호는 개인정보보호법상 고유식별정보라 원칙적으로 수집을 제한하므로 요청 항목에서 제외하고 본인확인 목적은 `birthday`로 대체한다. 로그인 식별자는 계속 이메일 또는 전화번호를 사용하며 별도 `아이디`(username) 필드는 추가하지 않는다.

(v6) `role=medication_subject`로 가입하면서 `guardian_phone`(보호자·지원인력 연락처)을 함께 보내면, 서버는 회원가입 처리와 같은 트랜잭션 흐름에서 `POST /care-relations/invitations`와 동일한 로직으로 초대를 자동 생성하고 응답에 `invitation_id`를 포함한다(`guardian_relation_type`을 생략하면 기본값 `guardian`). 초대 수락은 기존 2절의 절차(`/care-relations/invitations/{invite_token}/accept`)를 그대로 따른다 — 회원가입 시점에는 상대방이 미가입 상태일 수 있으므로 관계는 즉시 생성되지 않고 상대가 수락해야 `care_relations`가 만들어진다. 별도의 "연결되는 ID(기존 계정 식별자 직접 입력)" 방식은 두지 않는다 — 사용자에게 노출되는 로그인 아이디 개념이 없고, 전화번호 기반 초대가 미가입·가입 여부와 무관하게 동일하게 동작하기 때문이다.

(v6) 앱 최초 실행 화면 정책: 클라이언트에 유효한 `refresh_token`이 저장되어 있으면 `/auth/refresh`로 자동 로그인 후 홈 화면으로 이동한다. 저장된 토큰이 없거나 `/auth/refresh`가 `401 INVALID_REFRESH_TOKEN`을 반환하면 회원가입 화면을 기본으로 표시하고 "이미 계정이 있으신가요?" 링크로 로그인 화면에 진입할 수 있게 한다. 이 정책은 클라이언트 동작 규약이며 별도 API를 추가하지 않는다(REQ-001 부속).
...
| PATCH | `/medical-records/{record_id}/medications/{medication_id}` | 저신뢰 결과 확인·수정 | `{drug_name, dosage, frequency, diagnosis, drug_class, user_confirmed:true}` | `200 {medication_id, user_confirmed, typo_suggestion?}` |
...
(v6) 사용자가 저신뢰 결과를 직접 수정해 `PATCH .../medications/{medication_id}`를 호출하면, 서버는 입력된 `drug_name`을 약물명 사전과 대조해 정확히 일치하지 않으면 `typo_suggestion`(가장 유사한 후보 약물명, 예: "암로디민" 입력 시 `"암로디핀"`)을 응답에 포함한다. 사전에 일치하거나 충분히 유사한 후보가 없으면 `typo_suggestion`은 생략한다. 이 필드는 저장하지 않는 응답 전용 값이다. 사용자가 제안을 받아들이지 않고 `user_confirmed:true`로 재확인하면 그대로 확정한다. 확인 후에도 문제가 있으면(예: 사전에도 없는 약물, 반복적인 OCR 오류) 클라이언트는 해당 처방전 기록에 연결된 가이드가 이미 생성되어 있는 경우 `GET /medical-records/{record_id}/guide`의 `guide_result_id`로 `/chat`을 열 수 있는 "챗봇과 대화하기" 버튼을 노출한다(REQ-021~022). 가이드가 아직 없으면 챗봇 진입 버튼 대신 "확인 후 안내가 준비되면 챗봇에 물어볼 수 있어요" 안내만 표시한다 — `/chat` 요청 스키마 자체는 바꾸지 않는다.
