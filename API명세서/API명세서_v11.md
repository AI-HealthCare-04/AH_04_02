...
> 버전: v11 (2026-07-28). v10 대비 변경 내역은 `docs/revision_logs/API명세서/API명세서_v10_to_v11_revision_log.md` 참고.
>
> **v11은 v10(2026-07-22, 커밋 `8b1e574`) 이후 dev에 머지된 90개 커밋을 반영한다.** 자가진단(`assessments`) 기능과 `care_level_assessments` 테이블이 실제로 삭제됐고(REQ-005~007 "보류"→"구현취소"로 재정정), 보호자·기관의 처방전 검토·수정요청 흐름(REQ-063), 처방전 원본 사진 저장·열람(REQ-064), 계정 전환/세션 보안 재설계(REQ-065), 기관 연결해제 사유·승인 절차(REQ-066), 알림함(REQ-067), Web Push 백엔드(REQ-068, **프론트 미연동**), 다중 환자 모니터링 대시보드(REQ-069), 환자 관리 표 UI(REQ-070), 내 정보 자가수정(REQ-071), 회원가입 실시간 중복확인(REQ-072), 복약 일정 알림 대상 지정+일괄삭제(REQ-073), 등록내역 즐겨찾기(REQ-074), 최소 감사로그(REQ-081)가 신규로 추가됐다. 챗봇 DUR 인용 누락 버그, 생활습관 안내 KDCA 관련성 필터링 버그, 스케줄러 UTC/KST 버그, 초대 수락 시 기존 로그인 계정 재사용, 조직 계정의 caregiver/life_support_worker/social_worker 초대 수락 허용 등 다수 버그 수정도 포함한다.
...
| GET | `/monitoring/patients` | 내 정보(환자 본인) 또는 내가 케어하는 환자 목록 | 없음 | `200 [PatientPublic]`(**[v11]** `gender`, `diagnoses`(목록 조회 시만), `medication_status`, `today_status`(목록 조회 시만) 필드 추가) | O(`get_current_actor`) | 완료 |
...
| Method | Endpoint | 설명 | 요청 | 응답 | 인증 | 구현 |
|---|---|---|---|---|---|---|
| POST | `/invitations` | 초대 토큰 발급 — `relation_type`에 따라 두 방향을 모두 처리 | `{patient_id?, relation_type, invited_phone?, inviter_caregiver_id?}` | `200 {id, token, invite_url}`(**[v11]** `id` 추가 — 재발급 시 기존 초대 취소 후 재생성하는 흐름에 필요) | O(방향별로 다름, 아래 설명) | 완료 |
...
| GET | `/monitoring/caregivers/{caregiver_id}/patients` | 보호자가 연결된 환자 목록 — **[v11]** `revoked` 상태 링크 제외, 응답이 `PatientPublic`(진단명·복약상태·오늘상태 포함)으로 강화 | 경로 `caregiver_id` | `200 [PatientPublic]` | O | 완료 |
...
### 미구현 항목
- 초대 만료(`410 Gone`), 중복 처리 거부(`409`) — `expires_at` 필드는 v9에서 추가됐으나(초대 생성 시 발급+7일 계산), 만료 시 자동으로 `status="expired"`로 전환하는 별도 배치는 없고 `_get_invitation_by_token()`/`GET /caregivers/{id}/invitations` 호출 시점에만 지연 평가로 전환된다(요청이 안 오면 영원히 `pending`으로 남을 수 있음)
...
| GET | `/ocr/drug-info` | 약물상세 화면용 단건 조회 — **[v11]** 내부적으로 e약은요/HIRA/RAG 3종 하위 조회를 병렬화(ThreadPoolExecutor, 이전엔 순차) — 응답 필드 변경 없음, 응답 속도만 개선 | 쿼리 `drug_name` | `200 {drug_name, matched_name, drug_class, indication, precautions, side_effects, interactions, storage, patient_summary, dur_cautions}` | X(의도됨) | 완료 |
| GET | `/ocr/ping` | 라우터 연결 확인 | 없음 | `200 {status:"ok", owner:"권순현"}` | X | 완료(디버그용) |
| POST | `/ocr/test` | OCR 단독 테스트 | 쿼리 `patient_id`; multipart `file` | `200 {record_id, status, medications}`(각 medication에 `dose_amount` 포함) | O | 완료(테스트용) |
...
| Method | Endpoint | 설명 | 요청 | 응답 | 인증 | 구현 |
|---|---|---|---|---|---|---|
| POST | `/records/{record_id}/confirm` | 약물 항목 확정 → RAG 가이드 생성 → 복약일정 자동 등록. **[v11]** 같은 약이 다른 처방일자(`prescription_date`)로 재처방되면 예전 것을 중복으로 건너뛰지 않고 기존 일정을 대체(supersede), 응답에 `duplicate_drug_names` 추가. 완료 시점에 연결된 보호자/기관이 있으면 `caregiver_review_status="pending"`으로 시작하고 전원에게 검토요청 알림(REQ-063 참고, 없으면 `"none"`) | 경로 `record_id`; `{medications:[...]}`(각 항목에 `dose_amount` 포함 가능) | `200 {record_id, status, failure_reason, created_at, uploaded_by_name, medications:[...], guide:{medication_guide, lifestyle_guide, source_refs}, duplicate_drug_names:[str]}` | O | 완료 |
...
| GET | `/monitoring/patients/{patient_id}/notifications` | **[v11 신규, REQ-067]** 알림함(리마인더/누락 로그) — Push 프론트가 없는 지금은 사용자가 알림을 확인할 유일한 방법 | 경로 `patient_id`; 쿼리 `days?`(기본 30) | `200 [{id, schedule_id, drug_name, time_slot, due_date, kind, status, fired_at}]` | O | 완료 |
...
- `PATCH /notifications/{id}` 읽음 처리(알림함 자체는 REQ-067로 신규 추가됐으나 개별 항목 읽음 처리 API는 없음 — 조회 시 항상 최근 N일 전체를 보여줌)
...
- `patient_medications_router.py`는 v10 7절에 문서화 완료했다(v11에서 변경 없음).
