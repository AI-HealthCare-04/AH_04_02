...
- **날짜**: 2026-07-08
- **작성자**: 김영혜 (RAG 담당)
- **관련 문서**: `CONTRACT.md` 6번 항목([보류] 의약품 제품허가정보 API), `README_rag.md` "출처 1 / 출처 1-부가"
...
- **재발 방지**:
  - `CONTRACT.md`에 e약은요 vs 허가정보 vs HIRA 약가마스터 세 데이터 소스의 원천·용도·중복 관계를
    명시(`README_rag.md` "출처 1/출처 1-부가", `CONTRACT.md` 6번).
  - 허가정보 API 코드는 삭제 대신 `[보류]` 마커로 남겨, 실제 서비스명·엔드포인트·요청 파라미터·
    응답 필드(실측 검증됨)까지 `CONTRACT.md`에 기록해서 나중에 재검토·복원 시 처음부터 다시
    조사하지 않아도 되게 함.
...
- **재발 방지**: `rag-prototype`의 `SourceRef`/`GuideResponse`에 필드를 추가할 때마다
  ...
  없음), OCR↔RAG 계약처럼 RAG↔backend 계약도 필요하면 향후 `CONTRACT.md`에 추가 검토.
