...
쉬운 말로, 고령자도 이해할 수 있도록 짧은 문장으로 작성하세요.
"식사를 건강하게 하세요", "규칙적으로 운동하세요", "무리하지 마세요"처럼 무엇을 어떻게 하라는
건지 알 수 없는 막연한 조언은 절대 쓰지 마세요 — 그런 문장은 환자에게 실질적인 도움이 되지
않습니다. [참고자료]에 있는 구체적인 정보(음식·음료 이름, 조리법, 섭취량/횟수/시간, 운동 종류·
빈도·강도, 피해야 할 대상 등)를 최대한 그대로 살려서 "무엇을(대상) 어떻게(방법·양·빈도) 하라"는
형태로 쓰세요. [참고자료] 자체가 이미 구체적이면 요약하면서 그 구체성을 잃지 마세요. 반대로
[참고자료]에 구체적인 내용이 전혀 없어서 뭉뚱그린 문장 외에 쓸 방법이 없다면, 차라리 그 항목은
비워두세요(recommended/avoid에 넣지 마세요) — 막연한 문장을 억지로 채우는 것보다 낫습니다.
...
def _lifestyle_context_items(diagnosis: str | None) -> list[dict]:
    ...
    if diagnosis:
        ...
        for candidate_title in _candidate_kdca_titles(diagnosis):
            ...
            for doc in search_kdca_health_info_by_title(candidate_title):
                ...
                if doc_id in seen_kdca_ids or not _is_lifestyle_kdca_section(
                    ...
                    continue
                if not _title_matches_diagnosis(doc.metadata.get("title", ""), diagnosis):
                    continue
                seen_kdca_ids.add(doc_id)
                kdca_docs.append(doc)
