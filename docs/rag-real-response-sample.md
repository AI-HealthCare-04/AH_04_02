...
- 작성일: 2026-07-08
- 작성자: 김영혜
- 관련 PR: `feature/result-schema-compat_yunghye` → `dev`
...
- `frontend/src/pages/Result.tsx`: 어느 필드가 있는지 보고 스텁/실제 어느 모양인지
  판단해서 렌더링(`guide.lifestyle_guide.guides?.length`로 분기 등). **두 모드 다
  크래시 없이 동작** — `RAG_PROVIDER`를 stub↔real 아무 때나 전환해도 안전함.
