# RAG 실제 파이프라인 응답 샘플 & Result.tsx 호환 패치

- 작성일: 2026-07-08
- 작성자: 김영혜
- 관련 PR: `feature/result-prescdetail-schema-compat_yunghye` → `feature/frontend-setup_sojung`
  (최초 버전은 `dev` 대상 PR #12였는데, 소정님 브랜치가 그새 훨씬 앞서 있어서
  소정님 최신 브랜치 위로 다시 작성함 — 아래 "브랜치 상태 참고" 항목 확인)
...
결과 화면이 크래시**나는 걸 확인했다. `schedule_final_4days.md`의 D-3(박소정님이
`Result.tsx` 파싱 로직 수정)를 기다리면 병목이 생겨서, 실제 응답 모양을 이미 파악한
내가 직접 패치를 작성하고 소정님은 리뷰만 하는 것으로 역할을 조정했다.

패치 작성 도중 소정님이 같은 문제가 있는 `PrescriptionDetail.tsx`(처방상세 화면)를
추가로 알려주셔서 같이 고쳤다. `PrescriptionReview.tsx`(처방전확인/수정 화면)는
guide 데이터를 렌더링하지 않고 OCR 필드만 다뤄서 이 문제와 무관함을 확인했다.

## ⚠️ 브랜치 상태 참고 (중요)

`feature/frontend-setup_sojung`은 PR #11(`dev`, `57f73fb`)이 머지되기 **전** 지점에서
갈라져서, 아직 `run_rag_stub`(구버전, 동기 호출)을 쓰고 있고 `RAG_PROVIDER`/
`CHAT_PROVIDER` real 전환·HIRA `source_refs` 필드가 없다. 반대로 이 브랜치에만 있고
`dev`엔 없는 것도 있다(`POST /records/{id}/confirm`, 자가진단/초대/알림설정/챗봇
API 등). 두 브랜치를 합칠 때 `backend/routers/records_router.py`(import문 `run_rag_stub`
vs `run_rag`, `drug_code` 필드 유무)와 `backend/models.py`에서 실제 충돌이 날 걸로
보이니, 소정님 브랜치를 `dev`에 머지하시기 전에 이 부분 먼저 맞춰보면 좋겠습니다.
...
- `frontend/src/pages/Result.tsx` / `frontend/src/pages/PrescriptionDetail.tsx`: 어느
  필드가 있는지 보고 스텁/실제 어느 모양인지 판단해서 렌더링(`guide.lifestyle_guide.guides?.length`로
  분기 등). 두 화면 다 같은 로직 — **두 모드 다 크래시 없이 동작**, `RAG_PROVIDER`를
  stub↔real 아무 때나 전환해도 안전함.
- `PrescriptionReview.tsx`는 guide 데이터를 안 그려서(OCR 필드만 수정하는 화면) 이번
  패치 대상이 아님 — 확인만 하고 안 건드림.
