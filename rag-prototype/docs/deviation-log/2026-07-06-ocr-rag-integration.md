...
- **관련 문서**: `CONTRACT.md` (필드 계약 정본), `docs/troubleshooting-log/2026-07-06-ocr-confidence-review-required.md` (D2 상세 재현 기록)
...
- **무엇**: `MedicationItem.confidence=0.78`(OCR 자체 기준 0.80 미만 → 검토 필요)인 약을 실제 OpenAI 키로
  ...
  `docs/troubleshooting-log/2026-07-06-ocr-confidence-review-required.md` 참고.
...
- **재발 방지**: `CONTRACT.md` 2번 항목에 "왜 RAG가 OCR confidence를 다시 체크하는가(=OCR의 overall-only
...
- **재발 방지**: `CONTRACT.md` 3번 항목에 "다운스트림(`ai_worker` 등)에서 두 `review_required`를 함께
...
- **재발 방지**: `rag_chain.py`의 `DIAGNOSIS_DISEASE_ALIASES` 위 주석과 `CONTRACT.md` 4번 항목에 "커버
...
회귀 테스트 4건 + 계약 테스트 2건 추가, `CONTRACT.md` 신설).
